...
/* USER CODE BEGIN 0 */
/**
  * @brief Print a line on the debug console (host serial adapter, CR/LF appended).
  */
static void Debug_Log(const char *msg)
{
...
{
  uint8_t c;
  while (HAL_UART_Receive(&huart2, &c, 1, 50) == HAL_OK)
  {
  }
}
...
/**
  * @brief Arm interrupt-driven reception on the host UART (USART1).
  */
static void Serial_Command_Init(void)
{
  ...
  if (HAL_UART_Receive_IT(&huart1, &serial_rx_byte, 1U) != HAL_OK)
  {
    Error_Handler();
  }
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  (void)HAL_UART_Receive_IT(&huart1, &serial_rx_byte, 1U);
}

/**
  * @brief Recover host serial reception after an overrun/noise/framing error.
  */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  Serial_Command_Init();
  Debug_Log("new-board range bridge: booting");
     ...
     bytes received (if any) so wiring vs. baud problems can be told apart. */
  {
    ...
    for (;;)
    {
      ...
      attempt++;
      HAL_UART_Transmit(&huart2, (uint8_t *)"sys get ver\r\n", 13, 100);
      uint16_t n = 0;
      uint32_t t0 = HAL_GetTick();
      while (n < sizeof(raw) && (HAL_GetTick() - t0) < 1000U)
      {
        if (HAL_UART_Receive(&huart2, &raw[n], 1, 10) == HAL_OK)
        {
          n++;
        }
      }
  ...
  while (1)
  {
    ...
    /* A complete host serial line is handled only between LoRa receive windows, when
...
static void MX_USART2_UART_Init(void)
{
  ...
  huart2.Init.BaudRate = RN2483_BAUDRATE;
